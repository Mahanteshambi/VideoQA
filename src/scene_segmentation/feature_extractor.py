...
# from transformers import AutoImageProcessor, AutoVideoMAEModel # Older naming

# --- Device Configuration --- (already there)
# DEVICE = ...

# --- Load Video Model (e.g., VideoMAE) ---
try:
    # Using VideoMAE base model from Microsoft
    VIDEO_MODEL_CHECKPOINT = "MCG-NJU/videomae-base" 
    
    video_processor = VideoMAEImageProcessor.from_pretrained(VIDEO_MODEL_CHECKPOINT)
    video_model = VideoMAEModel.from_pretrained(VIDEO_MODEL_CHECKPOINT).to(DEVICE)
    video_model.eval() # Set to evaluation mode
    logger.info(f"Video model '{VIDEO_MODEL_CHECKPOINT}' loaded successfully for visual features.")
except Exception as e:
    logger.error(f"Failed to load video model: {e}. VLLM visual features will not be available.", exc_info=True)
    video_model = None
...
def get_vllm_visual_features_for_shot(
    shot_info: dict,
    ...
    """
    if not video_model or not video_processor:
        logger.warning("Video model not available. Skipping VLLM visual feature extraction.")
        return None
    ...
    if duration_sec <= 0:
        logger.warning(f"Shot {shot_info.get('shot_number', 'N/A')} has no duration. Skipping VLLM visual features.")
        return None
    ...
    try:
        ...
        # Sample frames evenly from the shot duration
        timepoints = np.linspace(start_sec, end_sec, num_frames_for_vllm, endpoint=True)
        
        for t in timepoints:
            frame_np = video_clip.get_frame(t)
            # Convert to RGB if needed
            if frame_np.shape[-1] == 4:  # RGBA
                frame_np = frame_np[..., :3]  # Keep only RGB
            # Convert numpy array to PIL Image for the processor
            frame_pil = Image.fromarray(frame_np)
            frames_list.append(frame_pil)

        video_clip.close()

        if not frames_list:
            logger.warning(f"No frames extracted for shot {shot_info.get('shot_number', 'N/A')} for VLLM.")
            return None

        # Process frames using the VideoMAE processor
        inputs = video_processor(frames_list, return_tensors="pt")
        inputs = {k: v.to(DEVICE) for k, v in inputs.items()}
        
        with torch.no_grad():
            outputs = video_model(**inputs)
            # Get the last hidden state as our feature vector
            feature_vector = outputs.last_hidden_state.mean(dim=1).squeeze().cpu().numpy()
        
        return feature_vector / (np.linalg.norm(feature_vector) + 1e-6)  # Normalize

    except Exception as e:
        logger.error(f"Error extracting VLLM visual features for shot {shot_info.get('shot_number', 'N/A')}: {e}", exc_info=True)
...
def extract_all_features_for_shot(
    ...
    # num_keyframes_for_visual: int = 1, # This was for CLIP-per-frame
    num_frames_for_vllm_visual: int = 16 # New parameter for the video model
) -> dict:
    """
    ...
    # VLLM-based Visual features
    features["visual"] = get_vllm_visual_features_for_shot(
        shot_info, 
        original_video_path,
        num_frames_for_vllm=num_frames_for_vllm_visual
    )
