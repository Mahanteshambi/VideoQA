...
def calculate_inter_shot_multimodal_similarity(
    ...
    Args:
        shot1_features (dict): Shot 1 dict containing multimodal features.
        shot2_features (dict): Shot 2 dict containing multimodal features.
    ...
    # The features are now directly in the dict, not under a 'features' key
    features1 = shot1_features
    features2 = shot2_features
    ...
    if "visual" in weights and weights["visual"] > 0:
        sim_v = calculate_cosine_similarity(features1.get("visual"), features2.get("visual"))
    ...
    if "textual" in weights and weights["textual"] > 0:
        sim_t = calculate_cosine_similarity(features1.get("textual"), features2.get("textual"))
    ...
    logger.debug(f"Shot {features1.get('shot_number','S1')} vs Shot {features2.get('shot_number','S2')} - TotalSim: {final_score:.3f}")
    return final_score